...
def price_storage_contract(
    ...
    """
    events = []
    # Dates ko explicit format ke sath parse kar rahe hain taaki warning na aaye
    for d, p in zip(injection_dates, purchase_prices):
        events.append((pd.to_datetime(d, format='%Y-%m-%d'), 'inject', p))
    for d, p in zip(withdrawal_dates, sale_prices):
        events.append((pd.to_datetime(d, format='%Y-%m-%d'), 'withdraw', p))
...
if __name__ == "__main__":
    ...
    try:
        # 1. Dataset load karein aur correct format define karein (Month/Day/Year)
        ...
        inj_prices = [price_map[pd.to_datetime(d, format='%Y-%m-%d')] for d in inj_dates]
        wth_prices = [price_map[pd.to_datetime(d, format='%Y-%m-%d')] for d in wth_dates]
